...
public class AddBoldTagString {
	...
	public static void main(String[] args) {
		AddBoldTagString ats = new AddBoldTagString();
		ats.test();
	}
	...
	public void test() {
		...
		String str = sol.addBoldTag(s, dict);
	    ...
	    public String addBoldTag(String s, String[] dict) {
	        ...
	        for(String str : dict) {
	        		while(s.indexOf(str, fromIndex) != -1) {
	        			map.put(s.indexOf(str, fromIndex), str);
	        			fromIndex = s.indexOf(str, fromIndex) + 1;
	        		}
	        		fromIndex = 0;
	        }
	        ...
	        for(Map.Entry<Integer, String> entry : map.entrySet()) {
	        		...
	        		if(start == -1 && end == -1) { // base case
	        			start = entry.getKey();
	        			end   = start + entry.getValue().length(); 
	        		} else {
	        			int key = entry.getKey();
	        			String value = entry.getValue();
	        			
	        			if(start <= key && key <= end) {
	        				end = Math.max(end, key + value.length());
	        			} else {
	        				arrays.add(new Integer[] {start, end});
	        				start = key;
	        				end   = key + value.length();
	        			}
	        		}
	        }
	        ...
	        for(Integer [] array : arrays) {
	        		if(start == -1 && end == -1) {
	        			if(array[0] > 0)
	        				bold.append(s.substring(0, array[0]));
	        			
	        			start = array[0];
	        			end = array[1];
	        			bold.append("<b>" + s.substring(start, end) + "</b>");
	        		} else {
	        			bold.append(s.substring(end, array[0]));
	        			start = array[0];
	        			end = array[1];
	        			bold.append("<b>" + s.substring(start, end) + "</b>");
	        		}
	        ...
	        bold.append(s.substring(end, s.length()));
